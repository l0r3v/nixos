...
in {
  ...
  systemd.tmpfiles.rules = [
    "f /var/lib/libvirt/qemu/nvram/haos_VARS.fd 0644 root root -"
  ];

  systemd.services.haos-vm-define = {
    ...
    script = ''
      ...
      <domain type='kvm'>
        <name>haos</name>
        <uuid>caf372dc-6964-4c9b-b60c-ca567f386df7</uuid>
        <description>Home Assistant OS</description>
        <memory unit='KiB'>2929688</memory>
        <currentMemory unit='KiB'>2929688</currentMemory>
        ...
        <os firmware='efi'>
          ...
          <nvram template='${pkgs.OVMF.fd}/FV/OVMF_VARS.fd' templateFormat='raw' format='raw'>/var/lib/libvirt/qemu/nvram/haos_VARS.fd</nvram>
          <boot dev='hd'/>
        </os>
        <features>
          <acpi/>
